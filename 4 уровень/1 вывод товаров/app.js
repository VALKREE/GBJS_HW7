...
function clickHandler(event) {
    ...
    category = event.currentTarget.attributes[0].nodeValue;
...
function showCategory(category) {
    if(category === "phones"){   
        products.phones.forEach(function(element){
            product = {
                productId: element.id,
                productImg: element.imageUrl,
    ...
    if(category === "tablets"){   
        products.tablets.forEach(function(element){
            product = {
                productId: element.id,
                productImg: element.imageUrl,
                productName: element.name,
    ...
    if(category === "tv"){   
        products.tv.forEach(function(element){
            product = {
                productId: element.id,
                productImg: element.imageUrl,
                productName: element.name,
...
function getProductMarkup(product) {
    ...
    <a href="https://example.com/producs/${product.productId}">Подробнее</a>`);
}